...
/**
 * Issue a request to start creating a changeset for a proposed stack update.
 *
 ...
 */
CloudFormation.prototype.createChangeSet = function (template, callback) {
  var params = {
    ChangeSetName: this.config.changeSetName,
    ChangeSetType: 'UPDATE',
    StackName: utilities.determineStackName(this.config),
  ...
  this.client.createChangeSet(params, function (error, data) {
    if (error) {
      return callback(new Error(util.format(
        'Call to createChangeSet failed: %s',
        error
      )));
    }
...
/**
 * Delete the changeset created for this given configuration.
 *
 * @param {String} changeSetName The changeset name.
 * @param {Function} callback Of the form function (error).
 */
CloudFormation.prototype.deleteChangeSet = function (callback) {
  var params = {
    ChangeSetName: this.config.changeSetName,
    StackName: utilities.determineStackName(this.config)
  };

  this.client.deleteChangeSet(params, function (error) {
    if (error) {
      return callback(new Error(util.format(
        'Call to deleteChangeSet failed: %s',
        error
      )));
    }
...
/**
 * Describe a changeset in order to obtain the necessary information on the
 * changes the update will produce.
 *
 ...
 *       ParameterKey: '',
 *       ParameterValue: '',
 *       UsePreviousParameterValue: true
 *     },
 *     ...
 *   ],
 *   CreationTime: Date,
 ...
 *     ...
 *   ],
 *   // Present for large responses that page across the list of changes.
 *   NextToken: ''
 * }
 *
 * @param {String} changeSetName The changeset name.
 * @param {Function} callback Of the form function (error, data).
 */
CloudFormation.prototype.describeChangeSet = function (callback) {
  var self = this;
  var data;

  /**
   * Recursively load pages for the describeChangeSet response.
   *
   * @param {String} [nextToken] The token to obtain the next page.
   */
  function loadPage (nextToken) {
    var params = {
      ChangeSetName: self.config.changeSetName,
      StackName: utilities.determineStackName(self.config)
    };
    ...
    self.client.describeChangeSet(params, function (error, dataPage) {
      if (error) {
        return callback(new Error(util.format(
          'Call to describeChangeSet failed: %s',
          error
        )));
      }

      // The assumption here is that only the Changes property is getting paged,
      // and that tags and parameters will not be effectively paged - all should
      // be in the first page.
      if (data) {
        data.Changes = data.Changes.concat(dataPage.Changes);
      }
      else {
        data = _.omit(dataPage, ['NextToken']);
      }

      if (dataPage.NextToken) {
        loadPage(dataPage.NextToken);
      }
      else {
        callback(null, data);
      }
...
CloudFormation.prototype.createStack = function (template, callback) {
  var params = {
    ...
    // Most stacks will need this, so may as well include it for all.
    Capabilities: this.config.capabilities,
    OnFailure: this.config.onDeployFailure,
    Parameters: utilities.getParameters(this.config),
    Tags: utilities.getTags(this.config),
...
/**
 * Issue a request to start updating a stack.
 *
 * Data returned has the form:
 *
 * {
 ...
 * @param {Function} callback Of the form function (error, data).
 */
CloudFormation.prototype.updateStack = function (template, callback) {
  var params = {
    StackName: utilities.determineStackName(this.config),
    // Most stacks will need this, so may as well include it for all.
    Capabilities: this.config.capabilities,
  ...
  utilities.addTemplatePropertyToParameters(params, template);
  this.client.updateStack(params, function (error, data) {
    if (error) {
      return callback(new Error(util.format(
        'Call to updateStack failed: %s',
        error
      )));
    }
